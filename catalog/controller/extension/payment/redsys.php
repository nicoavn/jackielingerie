...
class ControllerExtensionPaymentRedsys extends Controller {
	public function index() {
		...
		if ($this->config->get('redsys_entorno') == 'Real') {
			...
			$data['action'] = 'https://sis-d.redsys.es/sis/realizarPago/utf-8';		
		...
		$miObj->setParameter("DS_MERCHANT_TRANSACTIONTYPE", 0); //$trans always must be 0.
		...
		$miObj->setParameter("Ds_Merchant_Module","OP_redsys_3.1.0");
		
		///// 3DSecure
		if ($this->config->get('redsys_activar_3ds') == 'Si')
	...
	public function callback() {
		...
		$this->load->model('checkout/order');
		$estado_completado     = $this->config->get('redsys_estado_completo');
		$estado_cancelado      = $this->config->get('redsys_estado_cancelado');
